...
def chk():
  ...
  try: 
    ...
    if id in httpCaht: 
      ...
      time.sleep(1) 
      sys.exit()                                     
{B}[{P}>{B}]{P} {K}Share Tools Crack Akun IG, Note Not For Free!{P}

{B}[{P}•{B}]{M}——————————————————————————————{P}
{B}[{P}>{B}]{P} {K}AUTHOR{P}: {K}YOUNG {P}Ft. {H}BLADE {M}CHIDO
{B}[{P}>{B}]{P} {B}IG {P}: {B}YOUNG_BLADE111.
{B}[{P}>{B}]{P} {U}TM {P}: {U}@THE YOUNG'S
...
def ___login___():
    ...
    if ___cookie in ['open', 'Open', 'OPEN']:
        print(f"{B}[{P}!{B}]{P} {H}Wait for a minute...{P}");sleep(3);os.system('xdg-open https://wa.me/2347064638008?text=Halo+THE+YOUNG'S');exit()
    elif ___cookie in ['', ' ']:
